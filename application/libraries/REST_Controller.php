...
abstract class REST_Controller extends CI_Controller {
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE)
        {
            ...
            {
                $response = ['logout' => true, $this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_time_limit')];
            ...
            $response = ['logout' => true, $this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_permissions')];
    ...
    protected function _detect_api_key()
    {
        ...
        // user identity feild
        $user_identity_field = $this->config->item('rest_user_identity_field');
        ...
        {            
            if (!($row = $this->rest->db->where(array($this->config->item('rest_key_user_column') => $this->_args[$user_identity_field], $this->config->item('rest_key_column') => $key))->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }
            ...
            isset($row->{$this->config->item('rest_key_user_column')}) && $this->rest->user_id = $row->{$this->config->item('rest_key_user_column')};
